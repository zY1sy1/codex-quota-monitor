...
- 重置倒计时与本地时间；
- 当前连接、登录和数据新鲜度状态；
- 当前在用的 CC Switch provider（官方 default 或某个中转站）的“正在使用”圆点指示。
...
悬浮窗会在当前正在使用的额度行左侧显示一个小圆点（“正在使用”指示）。它读取 CC Switch 数据库的 `is_current` 标记（经由 `relay-quota-host.exe --inspect-cc-switch`）并按 **codex** 这一应用确定当前 provider：若是监视器里已配置的中转站，则点亮该中转站卡片旁的圆点；若是官方默认（`default`/官方），则点亮“5 小时额度”旁的圆点。该指示每约 60 秒刷新一次，只认 CC Switch 中 codex 应用的当前值；claude 等其他应用的当前 provider 不影响本指示。
...
托盘菜单中的“设置”打开设置中心，分为外观、行为和中转站三个页面：

- `外观`：显示模式、主题和完整窗口布局；
- `行为`：始终置顶、开机启动和立即刷新；
- `中转站`：打开中转站管理器，添加、测试和配置第三方额度来源。

设置会即时生效，不需要另行保存。切换浅色或深色主题时，设置中心自身和当前额度窗口会同步换肤；“完整窗口布局”仅在显示模式为完整窗口时可操作，切换到迷你条或额度球不会丢失已保存的布局。每个中转站在管理器内独立设置查询间隔，设置中心不再提供全局中转站查询间隔。设置中心与中转站、CC Switch 导入窗口一样显示应用图标。
