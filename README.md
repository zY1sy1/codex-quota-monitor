...
- 主题：浅色透明、深色透明；
- 完整窗口布局：总览折叠、标签切换；
- 自动查询间隔：`0–1440` 分钟，`0` 表示不自动查询中转站；
- 始终置顶；
- 开机启动；
- 立即刷新；
- 管理中转站。

模式、主题、完整窗口布局和自动查询间隔会持久化。修改自动查询间隔会立即重建中转站调度状态；其他外观切换只重绘当前快照，不额外触发 API 刷新。设置窗口与中转站、CC Switch 导入窗口一样显示应用图标。
...
“测试 provider”（旧界面称“测试脚本”）只执行一次手动验证并显示脱敏错误或归一化结果；“保存并启用”才会写入配置并加入自动调度。设置窗口中的全局自动查询间隔默认为 10 分钟，允许范围为 `0–1440`，并统一覆盖所有启用的中转站；设为 `0` 会关闭中转站自动查询，但“立即刷新”和手动测试仍可用。provider 文件中的旧 `IntervalMinutes` 字段仅为兼容保留，不再决定运行时调度间隔。全局并发上限为 2。单个 extractor 可以返回一个余额对象或多个套餐对象；不同单位只展示，不跨单位求和或比较。
