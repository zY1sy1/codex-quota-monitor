...
- Unit：`395/395` 通过。
- Integration：`167/167` 通过。
- EndToEnd：`9/9` 通过。
- Rust sidecar：`132/132` 通过，包含 Generic GET/POST/PUT 请求、结构化 Query/Header/Body、统一 origin 信任和远程明文 HTTP 拒绝。
- 覆盖：schema 1 → schema 2 provider 迁移、Generic/Custom 分流、凭据字段隔离、DPAPI 保护、日志/健康状态脱敏、sidecar 输出白名单、原始响应拒绝、官方/relay 隔离、缓存过期、429、超时、非法 JSON、并发上限和清洁退出。
...
- 解释：端点可达，认证后的套餐/余额验证必须由用户在管理窗口密码框输入真实凭据后执行“测试 provider”和“保存并启用”。凭据不得发送到聊天、命令行、日志、截图、健康状态或提交。
